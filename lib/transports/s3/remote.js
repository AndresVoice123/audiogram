...
module.exports = function(bucket, storagePath) {
  ...
  if (process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY) {
    const selectedBucket = process.env.S3_BUCKET ? process.env.S3_BUCKET : bucket;
    console.log('este es el bucket seleccionado', selectedBucket);
    var s3 = new AWS.S3({ params: { Bucket: selectedBucket } });
    // Test credentials
